...
func NewUpgrade(version, pkgURL string) *Upgrade {
	...
	u.SetIPtoHostName()
	var err error
	u.Client, err = kube.NewClient(kube.KubeDefaultConfigPath, nil)
	if err != nil {
...
func ExitUpgradeCase(version, pkgURL, cfgFile string) error {
	...
	if !FileExist(kube.KubeDefaultConfigPath) {
		return fmt.Errorf("KubeDefaultConfigPath %s is not exist, Exit", kube.KubeDefaultConfigPath)
	}

	if err := upgrade.Load(cfgFile); err != nil {
		upgrade.ShowDefaultConfig()
		return err
	}
...
func (u *Upgrade) upgradeNodes(hostnames []string, isMaster bool) {
	wg := NewPool(2)
	var err error
	for _, hostname := range hostnames {
		...
		go func(node string) {
			...
			k8sNode, _ := kube.GetNodeByName(u.Client, node)
			if kube.IsNodeReady(*k8sNode) {
				logger.Info("[%s] fourth:  %s nodes is ready", ip, node)

				// fifth to uncordon node
				err = kube.CordonUnCordon(u.Client, node, false)
				if err != nil {
