...
func CanUpgradeByNewVersion(new, old string) error {
	...
	// supkube change cri to containerd when version more than 1.20.0
	if newMajor == 120 && major == 119 {
		return fmt.Errorf("supkube change cri to containerd when Version greater than 1.20! New version: %s, current version: %s", new, old)
	}
